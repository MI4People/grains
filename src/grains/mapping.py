...
def create_mapping_prompt(section: Section, module: Module) -> str:
    ...
    module_info = f"Module: {module.name}\nTopics:\n" + "\n".join(
        f"- {topic.name}: {topic.description}" for topic in module.topics
    )
    return f"""
        Module:
        {module_info}
...
async def map_section_to_module_topics(section: Section, module: Module, agent: Agent) -> ModuleMappings:
    """
    Map a section to module topics
    """
    prompt = create_mapping_prompt(section, module)
    result = await agent.run(prompt)  # Use await with the async method
...
if __name__ == "__main__":
    ...
    if document:
        # mapping = asyncio.run(map_section_to_module_topics(document.sections[4], curriculum.modules[0], agent))
        all_mappings = asyncio.run(document_to_module_mapping(document, curriculum, agent))
        print(all_mappings)